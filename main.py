...
def load_iteration_count(config_path: Path = CONFIG_PATH) -> int:
	...
	try:
		return parser.getint("automation", "iter_count")
	except ValueError as exc:
		raise RuntimeError(f"Invalid iter_count value in {config_path}") from exc


def main() -> None:
	validate_adb_connection()
	iter_count = load_iteration_count()
